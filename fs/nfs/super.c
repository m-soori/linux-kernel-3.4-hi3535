...
out_no_data:
	printk("NFS: mount program didn't pass any mount data\n");
	return -EINVAL;

out_no_v3:
	printk("NFS: nfs_mount_data version %d does not support v3\n",
		 data->version);
	return -EINVAL;

out_no_sec:
	printk("NFS: nfs_mount_data version supports only AUTH_SYS\n");
	return -EINVAL;

#ifndef CONFIG_NFS_V3
out_v3_not_compiled:
	printk("NFS: NFSv3 is not compiled into kernel\n");
	return -EPROTONOSUPPORT;
#endif /* !CONFIG_NFS_V3 */

#ifndef CONFIG_NFS_V4
out_v4_not_compiled:
	printk("NFS: NFSv4 is not compiled into kernel\n");
	return -EPROTONOSUPPORT;
#endif /* !CONFIG_NFS_V4 */

out_nomem:
	printk("NFS: not enough memory to handle mount options\n");
	return -ENOMEM;
...
out_invalid_fh:
	printk("NFS: invalid root filehandle\n");
	return -EINVAL;
}
	...
	int flags, const char *dev_name, void *raw_data)
{
	...
	if (IS_ERR(server)) {
		...
		dfprintk(MOUNT,"NFS: nfs_create_server() failed  \n");
		goto out;
	}
